        ...
        if (h < minwsz)
          {
            c->isfloating = True;
            XRaiseWindow (dpy, c->win);
            resize (c, m->mx + (m->mw / 2 - WIDTH (c) / 2) + m->gappx,
...
tile2 (Monitor *m)
{
  ...
  for (clients_amt = 0, c = nexttiled (m->clients); c;
       c = nexttiled (c->next), clients_amt++)
    printf ("Client %d: [%s]\n", clients_amt, c->name);
  ...
  if (c) resize (c, m->wx, m->wy, m->ww - (2 * c->bw), m->wh - (2 * c->bw), 0);
